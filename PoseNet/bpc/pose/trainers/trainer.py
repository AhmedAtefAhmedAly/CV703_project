...
def train_pose_estimation(
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        train_deg_sum  = 0.0
        train_steps = 0
        epoch_total_images = 0  # Counter for images in this epoch

        tbar = tqdm(train_loader, desc=f"Train Epoch {epoch}/{epochs}", ncols=120)
        for batch in tbar:
            # Expect batch to be a tuple: (rgb, depths, polars, labels, metas)
            rgb_imgs, depth_list, polar_list, lbls, metas = batch

            # Move modalities to device.
            rgb = rgb_imgs.to(device)                     # Shape: [B, 3, H, W]
            depths = torch.stack(depth_list, dim=0).to(device)  # Shape: [B, 1, H, W]
            polars = torch.stack(polar_list, dim=0).to(device)  # Shape: [B, 1, H, W]

            batched_labels = batch_labels(lbls, device)

            optimizer.zero_grad()
            # Forward pass using multi-modal input.
            preds = model(rgb, depths, polars)
        ...
        with torch.no_grad():
            for batch in val_loader:
                rgb_imgs, depth_list, polar_list, lbls, metas = batch
                rgb = rgb_imgs.to(device)
                depths = torch.stack(depth_list, dim=0).to(device)
                polars = torch.stack(polar_list, dim=0).to(device)
                batched_labels = batch_labels(lbls, device)
                loss, metrics = criterion(batched_labels, model(rgb, depths, polars))
                val_steps += 1
        ...
        print(f"[INFO] Epoch: {epoch:03d} completed. Total images processed: {epoch_total_images}")
